...
//-------------------------------------------------------------------------
qt_cpu_calc_midle_class::qt_cpu_calc_midle_class() :
    arr_size(1024*1024),
    width(7)
{
...
//-------------------------------------------------------------------------
void qt_cpu_calc_midle_class::run(void)
{
    int x;
    int i;
    int *A;
    ...
    quint64 tick2;
    //-------------------------------------------------------------------------
    emit logMessage(QString::fromUtf8("Выполняем программу усреднения (%1 точек) на CPU в отдельном потоке (целочисленные вычисления)").arg(width));
    //-------------------------------------------------------------------------
    tick1 = 0;
    tick2 = 0;
    //-------------------------------------------------------------------------
    ...
    for(i = 0; i < 21; i++)
    {
        emit logMessage(QString::fromUtf8("%1 <-> %2").arg(A[i]).arg(C[i]));
    }
    //-------------------------------------------------------------------------
